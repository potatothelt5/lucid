...
import skid.krypton.module.modules.misc.StaffDetector;
...
public final class UndetectedTunnelBaseFinder extends Module {
    // Original TunnelBaseFinder settings
    ...
    // Undetected features
    private final BooleanSetting stealthMode = new BooleanSetting(EncryptedString.of("Stealth Mode"), true).setDescription(EncryptedString.of("Enable stealth mode to avoid detection"));
    private final BooleanSetting silentMining = new BooleanSetting(EncryptedString.of("Silent Mining"), true).setDescription(EncryptedString.of("Mine blocks silently without particles"));
    private final BooleanSetting autoShift = new BooleanSetting(EncryptedString.of("Auto Shift"), true).setDescription(EncryptedString.of("Automatically shift when mining"));
    private final NumberSetting miningDelay = new NumberSetting(EncryptedString.of("Mining Delay"), 0.0, 10.0, 2.0, 0.5).getValue(EncryptedString.of("Delay between mining actions"));
    private final BooleanSetting randomDelays = new BooleanSetting(EncryptedString.of("Random Delays"), true).setDescription(EncryptedString.of("Add random delays to appear more human"));
    private final BooleanSetting hideParticles = new BooleanSetting(EncryptedString.of("Hide Particles"), true).setDescription(EncryptedString.of("Hide mining particles"));
    private final BooleanSetting antiStaff = new BooleanSetting(EncryptedString.of("Anti Staff"), true).setDescription(EncryptedString.of("Stop mining when staff is nearby"));
    private final NumberSetting staffDetectionRange = new NumberSetting(EncryptedString.of("Staff Detection Range"), 10.0, 100.0, 30.0, 1.0).getValue(EncryptedString.of("Range to detect staff members"));
    ...
    private final StringSetting staffName7 = new StringSetting(EncryptedString.of("Staff Name 7"), "srmod").setDescription(EncryptedString.of("Seventh staff name to detect"));
    private final StringSetting staffName8 = new StringSetting(EncryptedString.of("Staff Name 8"), "sradmin ").setDescription(EncryptedString.of("Eighth staff name to detect"));
    ...
    private int stealthDelayCounter = 0;
    private int randomDelayCounter = 0;
    private boolean staffNearby = false;
    private int lastStaffCheck = 0;
    
    public UndetectedTunnelBaseFinder() {
        super(EncryptedString.of("Undetected Tunnel Base Finder"), EncryptedString.of("Finds bases digging tunnel (undetected)"), -1, Category.DONUT);
        this.addSettings(this.minimumStorage, this.spawners, this.autoTotemBuy, this.totemSlot, this.autoMend, this.xpBottleSlot, 
                        this.discordNotification, this.webhook, this.totemCheck, this.totemCheckTime,
                        this.stealthMode, this.silentMining, this.autoShift, this.miningDelay, 
                        this.randomDelays, this.hideParticles, this.antiStaff, this.staffDetectionRange,
    ...
    public void onEnable() {
        ...
        if (this.mc.player == null) {
            if (this.mc != null && this.mc.player != null) {
                this.mc.player.sendMessage(Text.of("UndetectedTunnelBaseFinder can only be enabled in-game!"));
            }
        ...
        this.sneaking = false;
        this.stealthDelayCounter = 0;
        this.randomDelayCounter = 0;
        this.staffNearby = false;
        this.lastStaffCheck = 0;
    }
    ...
    public void onDisable() {
        ...
        // Stop sneaking
        if (this.sneaking && this.mc.player != null) {
    ...
    public void onTick(final TickEvent event) {
        ...
        // Check for staff members if anti-staff is enabled
        if (this.antiStaff.getValue()) {
            this.checkForStaff();
            if (this.staffNearby) {
                this.stopMining();
                return;
        ...
        if (this.stealthMode.getValue() && this.randomDelays.getValue()) {
            if (this.randomDelayCounter > 0) {
                this.randomDelayCounter--;
                return;
            }
            // Add random delay every 100-300 ticks
            if (this.stealthDelayCounter % (100 + (int)(Math.random() * 200)) == 0) {
                this.randomDelayCounter = (int)(Math.random() * 60);
                this.stealthDelayCounter = 0;
            }
            this.stealthDelayCounter++;
        }
        ...
        try {
            ...
            // Use rotation logic from original TunnelBaseFinder
            if (this.mc.player != null) {
            ...
            System.err.println("UndetectedTunnelBaseFinder error: " + e.getMessage());
        }
    }

    private void checkForStaff() {
        if (this.lastStaffCheck > 0) {
            this.lastStaffCheck--;
            return;
        }
        
        this.lastStaffCheck = 20; // Check every second
        this.staffNearby = false;
        
        // Use the StaffDetector module if available
        try {
            StaffDetector staffDetector = (StaffDetector) Krypton.INSTANCE.MODULE_MANAGER.getModule(StaffDetector.class);
            if (staffDetector != null && staffDetector.isEnabled()) {
                this.staffNearby = staffDetector.isStaffNearby();
                return;
            }
        } catch (Exception e) {
            // Fall back to local detection if StaffDetector is not available
        }
        
        // Fallback local staff detection
        if (this.mc.world == null || this.mc.player == null) return;
        
        double range = this.staffDetectionRange.getValue();
        BlockPos playerPos = this.mc.player.getBlockPos();
        
        // Check for staff members using custom staff names
        this.mc.world.getPlayers().forEach(player -> {
            if (player != this.mc.player) {
                ...
                if (distance <= range * range) {
                    ...
                    // Check against all configured staff names
                    if (isStaffName(playerName)) {
    ...
    private boolean isStaffName(String playerName) {
        ...
        for (String staffName : staffNames) {
            if (!staffName.isEmpty() && playerName.contains(staffName)) {
                return true;
            }
        }
    ...
    private void handleBlockBreaking(final boolean b) {
        ...
        // Apply stealth mode
        if (this.stealthMode.getValue() && this.autoShift.getValue()) {
            this.ensureSneaking();
        }
            ...
            if (b && this.mc.crosshairTarget != null && this.mc.crosshairTarget.getType() == HitResult.Type.BLOCK) {
                ...
                if (!this.mc.world.getBlockState(blockPos).isAir()) {
                    ...
                    if (this.mc.interactionManager.updateBlockBreakingProgress(blockPos, side)) {
                        // Hide particles if enabled
                        if (!this.hideParticles.getValue()) {
    ...
    private void notifyFound() {
        ...
        if (this.spawnerCount > 10 && blockPos != null) {
            this.notifyFound("YOU FOUND SPAWNER (UNDETECTED)", blockPos.getX(), blockPos.getY(), blockPos.getZ(), false);
            this.spawnerCount = 0;
        }
        if (n > this.minimumStorage.getIntValue() && mc.player != null) {
            this.notifyFound("YOU FOUND BASE (UNDETECTED)", (int) this.mc.player.getPos().x, (int) this.mc.player.getPos().y, (int) this.mc.player.getPos().z, true);
        }
    }
    ...
    private void notifyTotemExploded(final String s, final int n, final int n2, final int n3) {
        if (this.discordNotification.getValue()) {
            ...
            bn.setTitle("Totem Exploded (Undetected)");
    ...
    private void disconnectWithMessage(final Text text) {
        ...
        final MutableText literal = Text.literal("[UndetectedTunnelBaseFinder] ");
        literal.append(text);
        this.toggle();
