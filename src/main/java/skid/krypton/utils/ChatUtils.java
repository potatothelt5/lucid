...
public final class ChatUtils {
    ...
    public static void updatePrefix() {
        String prefixText = "Krypton+";
        try {
            skid.krypton.module.modules.client.Krypton kryptonModule = (skid.krypton.module.modules.client.Krypton) Krypton.INSTANCE.getModuleManager().getModule(skid.krypton.module.modules.client.Krypton.class);
            if (kryptonModule != null) {
                prefixText = kryptonModule.chatPrefix.getValue();
            }
        } catch (Exception ignored) {
            // Use default if module not found
        }
        ...
        if (prefixText.equals("Krypton+")) {
            prefix.append(Text.literal("Kry").setStyle(Style.EMPTY.withFormatting(Formatting.GREEN)))
                  .append(Text.literal("ton+").setStyle(Style.EMPTY.withFormatting(Formatting.YELLOW)));
        } else {
            prefix.append(Text.literal(prefixText.substring(0, Math.min(prefixText.length(), 7))).setStyle(Style.EMPTY.withFormatting(Formatting.GREEN)))
                  .append(Text.literal(prefixText.length() > 7 ? prefixText.substring(7) : "").setStyle(Style.EMPTY.withFormatting(Formatting.YELLOW)));
        }
        
        PREFIX = prefix.append("] ");
    }

    public static Text getKryptonPrefix() {
        return PREFIX;
    }

    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned from the supplier the default Krypton prefix is used.
     */
    ...
    public static void sendPlayerMsg(String message) {
        Krypton.mc.inGameHud.getChatHud().addToMessageHistory(message);

        if (message.startsWith("/")) Krypton.mc.player.networkHandler.sendChatCommand(message.substring(1));
        else Krypton.mc.player.networkHandler.sendChatMessage(message);
    }
    ...
    public static void sendMsg(int id, String prefixTitle, Formatting prefixColor, Formatting messageColor, String messageContent, Object... args) {
        MutableText message = formatMsg(String.format(messageContent, args), messageColor);
        sendMsg(id, prefixTitle, prefixColor, message);
    }

    public static void sendMsg(int id, String prefixTitle, Formatting prefixColor, String messageContent, Formatting messageColor) {
        MutableText message = formatMsg(messageContent, messageColor);
        sendMsg(id, prefixTitle, prefixColor, message);
    }

    public static void sendMsg(int id, String prefixTitle, Formatting prefixColor, Text msg) {
        if (Krypton.mc.world == null) return;
        ...
        // Add to chat hud
        Krypton.mc.inGameHud.getChatHud().addMessage(message);
    }
    ...
    public static MutableText formatCoords(Vec3d pos) {
        ...
        // Add clickable coordinates (simplified version without Baritone integration)
        Style style = coordsText.getStyle().withFormatting(Formatting.BOLD)
            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                Text.literal("Click to copy coordinates")))
            .withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, 
                String.format("/tp %d %d %d", (int) pos.x, (int) pos.y, (int) pos.z)));
